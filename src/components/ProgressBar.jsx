...
const ProgressBar = ({sectionNum,setOpenSettings,settingsValues}) => {
    ...
    return (
			<div className={`fixed z-30 w-full h-[4rem] flex ${(sectionNum === 0 || !settingsValues["scroll_snap"])?"justify-end pr-10":"justify-center"}  ${(sectionNum > 1)?"bg-[#222222]":"bg-[#333333]"}`}>
                <div className="flex justify-center items-center lg:gap-10 gap-2">
                    ...
                    <>
                        ...
                        {sectionNum!==0 && <div className={`${sectionNum === 5?"text-text lg:text-3xl text-base":"text-gray-500 lg:text-2xl text-sm"} `}>
                            Projects
                        </div> }
                ...
                <FcSettings className="lg:text-3xl text-base cursor-pointer" onClick={()=>setOpenSettings(true)}/>
                </div>
			</div>
      );
}
