...
const Room = () => {
  ...
  const [initZoomLevel, setInitZoomLevel] = useState(
    sizes(height, width) * 0.2,
  );
    ...
    ctx.add(() => {
      gsap.to(three.scene.position, {
        ...
        scrollTrigger: {
          ...
          onUpdate: (trigger) => {
            setInitZoomLevel(
              trigger.progress * (sizes(height, width) - 0.7) + 0.7,
            );
  ...
  return (
    <PresentationControls snap={true} global={true}>
      <group dispose={null}>
        <mesh
          position={[0.0, 1.2, -2]}
          scale={
            initZoomLevel > sizes(height, width) * 0.2
              ? initZoomLevel
              : sizes(height, width) * 0.2
          }
          castShadow
